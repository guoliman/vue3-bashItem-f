...
import { getRolePage, updateRole, addRole, deleteRoles, getRoleMenuIds, updateRoleMenus, } from "@/api/role";
...
let formData = reactive<RoleForm>({
  sort: 1,
  status: 1,
  code: "",
  name: "",
  ...
  <div class="app-container">
    <div class="search">
      <el-form ref="queryFormRef" :model="queryParams" :inline="true">
        ...
        <el-form-item>
          <el-button v-hasPerm="['sys:role:get']" type="primary" @click="handleQuery"><i-ep-search />搜索</el-button>
    ...
    <el-card shadow="never">
      <template #header>
        <el-button v-hasPerm="['sys:role:add']" type="success" @click="openDialog()"><i-ep-plus />新增</el-button>
        <el-button v-hasPerm="['sys:role:delete']" type="danger" :disabled="ids.length === 0" @click="handleDelete()"><i-ep-delete />删除</el-button>
      </template>
      ...
      <el-table ref="dataTableRef" v-loading="loading" :data="roleList" highlight-current-row border
        ...
        <el-table-column fixed="right" label="操作" width="220">
          <template #default="scope">
            <el-button v-hasPerm="['sys:role:PostSonMenu']" type="primary" size="small" link @click="openMenuDialog(scope.row)">
              <i-ep-position />分配权限
            </el-button>
            <el-button v-hasPerm="['sys:role:edit']" type="primary" size="small" link @click="openDialog(scope.row)">
              <i-ep-edit />编辑
            </el-button>
            <el-button v-hasPerm="['sys:role:delete']" type="primary" size="small" link @click="handleDelete(scope.row.id)">
              <i-ep-delete />删除
            </el-button>
          </template>
        </el-table-column>
