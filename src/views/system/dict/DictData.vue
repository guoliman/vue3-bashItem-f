...
import { DelId,DictPageVO, DictForm, DictQuery } from "@/api/dict/types";
...
const props = defineProps({  // defineProps定义变量 用于接收父组件传递的值 DictData.vue是子组件 
  type_code: {
    type: String,
    default: () => {
      return "";
...
watch(() => props.type_code,(newVal: string) => {
    queryParams.type_code = newVal;
    formData.type_code = newVal;
    resetQuery();
  }
);
...
let formData = reactive<DictForm>({
  status: 1,
  type_code: props.type_code,
  sort: 1,
});
 ...
 * @param dictId 字典ID
 */
function openDialog(row?: DictForm) {
  dialog.visible = true;
  if (row) {
    dialog.title = "修改字典";
...
function handleDelete(dictId?: number) {
  delId.id = ""
  delId.id = [dictId || ids.value].join(",");
  if (!delId.id) {
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="500px" @close="closeDialog">
      <el-form ref="dataFormRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <el-form-item label="字典名称" prop="name">
          <el-input v-model="formData.name" placeholder="请输入字典名称 (zh-cn)" />
        </el-form-item>
        <el-form-item label="字典值" prop="value">
          <el-input v-model="formData.value" placeholder="请输入字典值 (en)" />
        </el-form-item>
