...
import {
  getDeptRole, getUserPage, getUserForm, deleteUsers, addUser, updateUser, updateUserStatus, updateUserPassword,
  downloadTemplateApi, exportUser, importUser
} from "@/api/user";
...
const genderList: Record<number, string> = reactive({
  0: "未知",
  1: "男",
  2: "女"
});
...
const deptRoleList = reactive<any>({
  dept: {},
  role: {},
})
...
let formData = reactive<UserForm>({
  status: 1,
});
 ...
 * 获取部门和角色数据 用于展示数据
 */
function showDeptRole() {
  loading.value = true;
  getDeptRole()
    .then(({ data }) => {
      // deptRoleList.dept = data.dept
      // deptRoleList.role = data.role
      Object.assign(deptRoleList, data);  // 赋值 把data赋值给deptRoleList
      // console.log("deptRoleList===",deptRoleList)
    })
    .finally(() => {
      loading.value = false;
    });
  ...
  ElMessageBox.prompt(
    ...
    .then(({ value }) => {
      if (!value || value.trim().length <6) {
        ElMessage.error("密码长度不能小于6 请从新更改");
        return false;
      }
 ...
 * @param userId 用户ID
 */
async function openDialog(row?: UserForm) {
...
function handleDelete(id?: number) {
  // const ida = ref( [id || ids.value].join(","));
  const ida = ref();
  ida.value =  [id || ids.value].join(",")
...
onMounted(async () => {
  // await getDeptOptions(); // 异步-串行
  await Promise.all([    // 异步-并发 Promise.all内都是并发
    showDeptRole(),   // 获取部门和角色
    getDeptOptions(), // 获取展示页左侧角色
  ]);

  await handleQuery();   // 异步-串行 初始化用户列表 
});
      ...
      <el-col :lg="4" :xs="24" class="mb-[12px]">
        <el-card shadow="never">
          ...
          <el-tree ref="deptTreeRef" class="mt-2" :data="deptList"
            ...
            :filter-node-method="handleDeptFilter" default-expand-all @node-click="handleDeptNodeClick"></el-tree>
        </el-card>
      </el-col>
      ...
      <el-col :lg="20" :xs="24">
        <div class="search">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item>
              <el-button v-hasPerm="['sys:user:get']" type="primary" @click="handleQuery"><i-ep-search />搜索</el-button>
        ...
        <el-card shadow="never">
          ...
          <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
            ...
            <!-- <el-table-column key="id" label="编号id" align="center" prop="id" width="100" /> -->
            ...
            <el-table-column label="角色" width="220" align="center" prop="roleIds">
              <template #default="scope">
                <el-tag type="warning" v-for="(k, index) in scope.row.roleIds" :key="index" style="margin-right: 2px;">{{
                  deptRoleList.role[k] }}</el-tag>
              </template>
            </el-table-column>

            <el-table-column label="部门" width="120" align="center" prop="deptId">
              <template #default="scope">{{ deptRoleList.dept[scope.row.deptId] }}</template>
            </el-table-column>
            ...
            <el-table-column label="性别" width="100" align="center" prop="genderLabel">
              <template #default="scope">{{ genderList[scope.row.gender] }}</template>
            </el-table-column>
            ...
            <el-table-column label="操作" fixed="right" width="220">
              <template #default="scope">
                ...
                <el-button v-hasPerm="['sys:user:edit']" type="primary" link size="small"
                  @click="openDialog(scope.row)"><i-ep-edit />编辑</el-button>
