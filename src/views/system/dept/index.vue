...
import { DeptDel, DeptVO, DeptForm, DeptQuery } from "@/api/dept/types";
defineOptions({
  name: "Dept",
  inheritAttrs: false,
});
...
let formData = reactive<DeptForm>({
  status: 1,
  parentId: 0,
  sort: 1,
});
 ...
 * @param deptId 部门ID
 */
async function openDialog(parentId?: number, row?: DeptForm) {
  await getDeptOptions();
  dialog.visible = true;
  if (row) {
    dialog.title = "修改部门";
    Object.assign(formData, row);  // 把row的值复制给formData
...
function handleDelete(deptId?: number) {
  ...
  ElMessageBox.confirm(`确认删除已选中的数据项?`, "警告", {
    ...
    const idsList = reactive<DeptDel>({ id: deptIds })
    deleteDept(idsList).then(() => {
    ...
    <div class="search">
      <el-form ref="queryFormRef" :model="queryParams" :inline="true">
        <el-form-item label="关键字" prop="name">
          <el-input v-model="queryParams.name" placeholder="部门名称" @keyup.enter="handleQuery" />
        </el-form-item>
        ...
        <el-form-item>
          <el-button v-hasPerm="['sys:dept:get']" class="filter-item" type="primary" @click="handleQuery">
            <i-ep-search />
            搜索
          </el-button>
        ...
        <el-table-column label="操作" fixed="right" align="left" width="200">
          <template #default="scope">
            ...
            <el-button v-hasPerm="['sys:dept:edit']" type="primary" link size="small"
              @click.stop="openDialog(scope.row.parentId, scope.row)"><i-ep-edit />编辑
            </el-button>
